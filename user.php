...
switch ($op) {
    ...
    case 'user_login':
        ...
        if ($login) {
			...
			echo "<script> alert('帳號密碼不符，請確認帳號密碼是否有誤?'); </script>";
...
function tweets_list($user_sn='')
{
    ...
    $sql     = "SELECT * FROM `tweets` where `user_id`='{$user_id}' ORDER BY `tweets_date` desc";
    $result  = $mysqli->query($sql) or die($mysqli->connect_error);
    ...
    $smarty->assign('user_sn', $user_sn);
}
function collects_list($user_sn='')
{
    global $mysqli, $smarty, $isAdmin, $isUser;
	if (empty($user_sn)) {
        $user_sn = $_SESSION['user_sn'];
    ...
    $sql     = "SELECT * FROM `collects` where `user_sn`='{$user_sn}' ORDER BY `collects_date` desc";
    $result  = $mysqli->query($sql) or die($mysqli->connect_error);
