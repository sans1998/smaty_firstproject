...
function get_goods_pic($goods_sn = '', $type = "goods")
{
    ...
    if (file_exists($filename)) {
        ...
        return "http://dummyimage.com/{$size}/889dd1/ffffff.gif&text=無景點照片";
    }
}
//取得圖片
function get_tweets_pic($tweets_sn = '', $type = "tweets")
{
    $filename = "uploads/{$type}/{$tweets_sn}.png";
    if (file_exists($filename)) {
        return $filename;
    } else {
        $size = ($type == 'tweetsthumbs') ? "300x200" : "600x400";
        return "http://dummyimage.com/{$size}/889dd1/ffffff.gif&text=無推文照片";
    }
}